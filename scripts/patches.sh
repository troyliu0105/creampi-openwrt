...
case $status_page in
  *htm)
    ...
    sed -i $line_number_FV' a <a href="https://github.com/troyliu0105/creampi-openwrt" target="_blank">troyliu0105/creampi-openwrt</a> '$strDate $status_page
    ;;
  *js)
    line_number_FV=`grep -m1 -n 'var fields' $status_page | cut -d: -f1`
    sed -i $line_number_FV' i var pfv = document.createElement('\''placeholder'\'');pfv.innerHTML = '\''<a href="https://github.com/troyliu0105/creampi-openwrt" target="_blank">troyliu0105/creampi-openwrt</a> '$strDate"';" $status_page
    line_number_FV=`grep -n 'Firmware Version' $status_page | cut -d : -f 1`
